...
def test_export_suppliers_csv(client, auth, db):
    # Create a test admin user
    from app.models.user import User
    User.create(db, "admin", "password123", "Admin User", "admin")
    
    auth.login()
    response = client.get('/suppliers/export')
    assert response.status_code == 200
    assert response.mimetype == 'text/csv'
    assert 'attachment; filename=suppliers.csv' in response.headers['Content-Disposition']
